<%--
  Created by IntelliJ IDEA.
  User: xiaod
  Date: 2018/6/12
  Time: 14:24
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" import="java.sql.*" %>
<html>
<head>
    <title>Title</title>
</head>
<body>
<%
    String num= (String) request.getAttribute("NUMBER");
    String nam=(String)request.getAttribute("NAME");
    String sco=(String)request.getAttribute("SCORE");

    Connection conn = null;
    Statement stat = null;
    PreparedStatement ps = null;
    ResultSet res = null;

    String sql = "select number,name,score from student1;";
    try {
        Class.forName("com.mysql.jdbc.Driver");
        System.out.println("JDBC加载成功");
    } catch (Exception e1) {
        System.out.println("JDBC加载失败");
        e1.printStackTrace();
    }
    try {
        conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/student?useSSL=false&serverTimezone=UTC",
                "root", "long8772521");

        stat = conn.createStatement();
        res = stat.executeQuery(sql);
        while(res.next()){
            if(res.getString(1).equals(num)){
                try{
                    Class.forName("com.mysql.jdbc.Driver");
                    System.out.println("JDBC加载成功");
                }catch (Exception e1){
                    System.out.println("JDBC加载失败");
                    e1.printStackTrace();
                }
                String sql2 = "update student1 set name = '"+nam+"' where number = '"+num+"'" ;
                String sql3 = "update student1 set score = '"+sco+"' where number = '"+num+"'";
                try{
                    conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/student?useSSL=false&serverTimezone=UTC","root", "long8772521");
                    stat = conn.createStatement();
                    stat.executeUpdate(sql2);
                    stat.executeUpdate(sql3);
                    System.out.println("MySQL链接成功");
                }catch (SQLException g) {
                    System.out.println("MySQL链接失败");
                    g.printStackTrace();
                }
            }

        }

        } catch (SQLException e) {
        e.printStackTrace();
    }

    finally {
        try{
            conn.close();
            System.out.println("MySQL关闭成功！");
        }catch (SQLException e2){
            System.out.println("MySQL关闭成功！");
            e2.printStackTrace();
        }
    }
%>

<BR>
<div align="center">
    <table width="100%">
        <tr>
            <td>
                <div align="center">修改成功！</div>
            </td>
        </tr>
        <tr>
            <td>
                <div align="center"><a href="index.jsp">点击返回主界面 </a> </div>
            </td>
        </tr>
    </table>
</div>
</body>
</html>
